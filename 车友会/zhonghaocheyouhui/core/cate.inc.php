...
function getCateById($id){
	$sql="select id,cName from car_cate where id={$id}";
	return fetchOne($sql);
}
...
function delCate($id){
	...
	if(!$res){
		...
		alertMes("不能删除分类，请先删除该分类下的商品", "listAct.php");
	}
}
 ...
 */
function getAllCate(){
	$sql="select id,cName from car_cate";
	$rows=fetchAll($sql);
	return $rows;
}
